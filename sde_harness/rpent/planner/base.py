...
def build_planner(
    ...
    *,
    ...
    if planner_type in {"cursor", "cursor_sdk"}:
        from rpent.planner.cursor import CursorSdkPlanner

        cu_timeout_s = planner_timeout_s
        if cu_timeout_s is None:
            cu_timeout_s = int(
                os.environ.get(
                    "CURSOR_TIMEOUT_S",
                    os.environ.get("CELL_TIMEOUT_S", "1200"),
        ...
        return CursorSdkPlanner(
            output_dir=output_dir,
            repo_root=get_repo_root(),
            model=model,
            timeout_s=cu_timeout_s,
